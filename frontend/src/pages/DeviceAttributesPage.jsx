...
export function DeviceAttributesPage() {
  ...
  const handleDelete = async (attr) => {
    if (!window.confirm(vi.deviceAttributes.deleteConfirm(attr.attribute_name)))
      return;
    setSubmitting(true);
    try {
    ...
    <div className="space-y-6">
      ...
      >
        <p className="mb-4 text-sm text-slate-600">
          {vi.deviceAttributes.intro}
        </p>

        {error && (
          <p className="mb-4 text-sm text-red-600">{getErrorMessage(error)}</p>
        )}
        {loading ? (
              ...
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-3 py-2 text-left font-medium text-slate-700">
                    {vi.deviceAttributes.colName}
                  </th>
                  <th className="px-3 py-2 text-left font-medium text-slate-700">
                    {vi.deviceAttributes.colValue}
                  </th>
                  {isOwner && (
                    ...
                    {isOwner && (
                      <td className="whitespace-nowrap px-3 py-2 text-right">
                        ...
                        <Button
                          type="button"
                          variant="danger"
                          className="ml-2 !py-1 !text-xs cursor-pointer"
      ...
      <Modal
        ...
        title={
          editAttr
            ? vi.deviceAttributes.modalEdit
            : vi.deviceAttributes.modalAdd
        }
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              className="input-box min-h-[100px]"
              value={value}
