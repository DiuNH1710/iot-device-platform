    ...
    <div className="flex min-h-screen bg-slate-50">
      <aside className="hidden w-56 shrink-0 border-r border-slate-200 bg-white md:block">
        <div className="flex h-14 items-center border-b border-slate-200 px-4">
          <span className="font-semibold text-slate-900">IoT Console</span>
        </div>
        <nav className="space-y-1 p-3">
          <NavLink to="/dashboard" className={navCls} end>
            <IoGridOutline className="h-5 w-5" /> Dashboard
          </NavLink>
          <NavLink to="/devices" className={navCls}>
            <IoHardwareChipOutline className="h-5 w-5" /> Devices
          </NavLink>
          <NavLink to="/alerts" className={navCls}>
            <IoNotificationsOutline className="h-5 w-5" /> Alerts
          </NavLink>
        </nav>
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 md:px-6">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="hidden text-sm text-slate-600 sm:inline">{username || 'User'}</span>
            <button
              type="button"
