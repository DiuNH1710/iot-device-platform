...
export function TelemetryChart({ telemetry, metricKey, loading }) {
  ...
  // sửa  chỉ refresh khi có data mới
  const chartData = useMemo(() => {
    return telemetry
      .map((row) => {
        const value = getNumericValueFromData(row?.data, metricKey);
    ...
    <div className="h-80 w-full">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Tooltip
            labelFormatter={(_, p) =>
              p?.[0]?.payload?.time ? formatTime(p[0].payload.time) : ""
            }
